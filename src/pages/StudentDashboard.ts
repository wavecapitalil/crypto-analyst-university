...
export async function studentDashboard(){
  if(!authStore.configured)return `<div class="section"><h1>My Dashboard</h1><div class="callout amber">מערכת הדאשבורד מוכנה, אבל נדרש backend מחובר כדי ליצור חשבונות אמיתיים ולסנכרן בין מכשירים.</div><button class="btn" data-route="account">Student Account</button></div>`;
  if(!authStore.session)return `<div class="section"><h1>My Dashboard</h1><div class="callout">יש להתחבר כדי לפתוח את הדאשבורד האישי.</div><button class="btn green" data-route="account">הרשמה / התחברות</button></div>`;
  ...
  const academyRows=m.academies.map((a:any[])=>{const p=academyPct(m,a[2]);return `<div class="skill"><b>Academy ${safe(a[0])} — ${safe(a[1])}</b><div class="small">${p}%</div><div class="skillbar"><i style="width:${p}%"></i></div></div>`}).join('');
