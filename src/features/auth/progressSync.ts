...
function ts(x?:string){const n=x?Date.parse(x):0;return Number.isFinite(n)?n:0}
function hasLearningData(p:ProgressState){
  ...
  try{
    ...
    if(!remoteRaw){
      ...
      if(remoteTs>localTs || (!localHas&&remoteHas)){
        ...
        // Equal/unknown timestamps: keep local to avoid silently overwriting this device.
        await upsertRemoteProgress(session,local);
      }
    }
