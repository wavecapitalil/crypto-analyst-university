...
import {emptyProgress} from './schema.js'; import {migrateProgress} from './migrations.js';
const KEY='wave_crypto_uni_v10'; const LEGACY=['wave_crypto_uni_v7'];
...
export function loadProgress():ProgressState{
 for(const key of [KEY,...LEGACY]){const raw=get(key);if(raw){try{const state=migrateProgress(JSON.parse(raw));saveProgress(state);return state}catch{}}}
...
export function importProgress(json:string){return migrateProgress(JSON.parse(json))}
