...
export async function render() {
    ...
    try {
        ...
        const detail = app.querySelector('#render-error-detail');
        if (detail)
            detail.textContent = e instanceof Error ? e.message : 'Unknown application error';
    }
}
...
function calculate(tool) {
    ...
    const vars = {};
    tool.querySelectorAll('input[data-calc-var]').forEach(x => vars[x.dataset.calcVar] = Number(x.value || 0));
...
export function bindInteractions() {
    document.addEventListener('click', async e => {
        ...
        switch (el.dataset.action) {
            ...
            case 'export-progress': {
                ...
                const url = URL.createObjectURL(new Blob([JSON.stringify(progressStore.state, null, 2)], { type: 'application/json' }));
                a.href = url;
                a.download = 'Wave_Capital_University_v11_progress.json';
                a.click();
    ...
    document.addEventListener('input', e => {
        ...
        const tool = target.closest('.tool[data-tool]');
        if (tool)
            calculate(tool);
    });
    document.addEventListener('keydown', e => {
        const t = e.target;
        if (t.id === 'nav-search' && e.key === 'Enter')
